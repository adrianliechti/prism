...
export function ResponseViewer() {
  ...
  if (!response) {
    return (
      <div className="flex items-center justify-center p-12">
        <p className="text-sm text-gray-500">Enter a URL and click Send to see the response</p>
      </div>
    );
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-400">Header</th>
            ...
            <tbody>
              {Object.entries(response.headers).map(([key, value]) => (
                <tr key={key} className="border-b border-white/5 last:border-0">
                  <td className="px-4 py-2 font-mono text-xs text-gray-300">{key}</td>
                  <td className="px-4 py-2 font-mono text-xs text-gray-400">{value}</td>
                </tr>
              ))}
            </tbody>
          </table>
